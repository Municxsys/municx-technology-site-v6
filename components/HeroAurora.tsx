...
export default function HeroAurora({ children }: { children: React.ReactNode }) {
  ...
  useEffect(()=>{
    ...
    let a=0, raf=0;
    const wave=(yb,amp,freq,ph,col,al)=>{const r=c.getBoundingClientRect(),w=r.width,h=r.height;ctx.save();ctx.globalAlpha=al;ctx.beginPath();for(let x=0;x<=w;x+=2){const y=yb+Math.sin((x*freq)+ph)*amp; if(x===0)ctx.moveTo(x,y); else ctx.lineTo(x,y);}ctx.strokeStyle=col;ctx.lineWidth=2;ctx.stroke();ctx.restore();};
